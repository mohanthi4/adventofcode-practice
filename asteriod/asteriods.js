const opertions = (instructions, position,currentPosition, action) => {
  const storageIndex = currentPosition[0] === 0 ? instructions[position + 3]: position + 3 ;
  const parameter1 = currentPosition[2] === 0 ? instructions[position + 1]: position + 1;
  const parameter2 = currentPosition[1] === 0 ? instructions[position + 2] : position + 2;
...
const singleValuedOperations = (instructions, position, currentPosition,action, value = 1)=>{
  const storage = currentPosition[2] === 0 ? instructions[position + 1] : position + 1;
  switch (action) {
...
const jumpOperations = (instructions, position, currentPosition, action) => {
  const index1 = currentPosition[2] === 0 ? instructions[position + 1] : position + 1;
  const pointer = currentPosition[1] === 0 ? instructions[instructions[position + 2]] : instructions[position + 2];
  switch (action) {
...
const instruction = input.split(",").map((x) => parseInt(x));
console.log(executeInstructions(instruction, 0));